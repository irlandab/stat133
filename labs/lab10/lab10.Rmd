...
#library(stringr)
#library(ggmap)
library(plotly, dependencies = TRUE)
```
...
Now that you have two vectors `latitude` and `longitude`, and the corresponding 
...
Notice the use of the tildes next to `x` and `y` arguments. This is what `plot_ly()` uses to __map__ the values of a column (in a data.frame) as the attributes of a 
graphical element.
...
```{r sf_map_burritos, echo = FALSE, warning = FALSE, eval = FALSE}
